...
class EnumImpl<const TEnumData extends EnumData = EnumData> implements EnumMethods<TEnumData> {
     ...
     * @param {number|string} enumData[].value - Value for the enum
    ...
    _setEnumData(enumData: TEnumData) {
        ...
        enumData.forEach((singleEnum) => {
            ...
            // Validate enum value - have to be number or string
            if (typeof singleEnum.value !== 'number' && typeof singleEnum.value !== 'string')
                throw new TypeError('Enum value have to be number or string value!');
    ...
    _setEnumDataToArray(enumData: TEnumData) {
        ...
        enumData.forEach((singleEnum) => {
            ...
            if (this._array.find((e) => e.value === singleEnum.value) !== undefined)
                throw new TypeError('Enum already contains an object with same value!');

            this._array.push(singleEnum);
        });
    }
     ...
     * 
     * @param {number|string} enumValue - The value of the wanted enum.
     * 
     * @memberOf Enum
     */
    get<TValue extends EnumItem<TEnumData>['value']>(enumValue: TValue): EnumByValue<TEnumData, TValue>;
    ...
    get(enumValue: EnumValue) {
        // Get enum object from array by value
        const enumObject = this._array.find((e) => e.value === enumValue);
